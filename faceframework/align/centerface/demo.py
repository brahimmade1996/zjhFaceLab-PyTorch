...
def test_image(file):
    ...
    t1=time.time()
    if landmarks:
        dets, lms = centerface(frame, h, w, threshold=0.55)
    else:
    ...
    for det in dets:
        ...
        cv2.rectangle(frame, (int(boxes[0]), int(boxes[1])), (int(boxes[2]), int(boxes[3])), (2, 255, 0), 1)
    t2=time.time()
    print('detected time%f cnt:%d'%(t2-t1,len(dets)))
    if landmarks:
        for lm in lms:
            for i in range(0, 5):
                cv2.circle(frame, (int(lm[i * 2]), int(lm[i * 2 + 1])), 2, (0, 0, 255), -1)
...
if __name__ == '__main__':
    #camera()
    #imgfile='../../../data/test/2.jpg'
    #test_image(imgfile)

    imgfile='../../../data/test/20200628172651raw.jpg'
    test_image(imgfile)

    imgfile='../../../data/test/20200628172906raw.jpg'
    test_image(imgfile)

    imgfile='../../../data/test/11.jpg'
    test_image(imgfile)
    # test_widerface()
