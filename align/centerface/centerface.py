...
class CenterFace(object):
    def __init__(self, landmarks=True):
        ...
        if self.landmarks:
            onnxpath = basepath+'/model/onnx/centerface.onnx'
            self.net = cv2.dnn.readNetFromONNX(onnxpath)
        else:
            self.net = cv2.dnn.readNetFromONNX(basepath+'/models/onnx/cface.1k.onnx')
